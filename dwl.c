...
#define VISIBLEON(C, M)         ((M) && (C)->mon == (M) && ((C)->tags & (M)->tagset[(M)->seltags]) && !(C)->ishidden && !(C)->swallowedby)
...
buttonpress(struct wl_listener *listener, void *data)
{
	...
	case WL_POINTER_BUTTON_STATE_PRESSED:
		...
		if (click == ClkClient && c && (!client_is_unmanaged(c) || client_wants_focus(c))) {
...
focusclient(Client *c, LayerSurface *l, int lift)
{
	...
	if (focused_client && focused_client != c && !(c && client_is_unmanaged(c))) {
		...
		focused_client = NULL;
        if (focused_client && focused_client->tempshow) {
            focused_client->tempshow = 0;
            focused_client->ishidden = 1;
            arrange(focused_client->mon);
        }
	}
	drawbars();
